    ...
    <main>
      ...
      >
        ...
        <div className="relative z-10 max-w-3xl" style={{ animation: "heroUp .8s cubic-bezier(0.16,1,0.3,1) both" }}>
          <div className="flex items-center gap-3 mb-5">
            ...
            <span className="font-mono text-[11px] text-[var(--accent)] tracking-[0.12em] uppercase">Our Services</span>
          </div>
          <h1 className="font-syne text-[clamp(40px,6vw,80px)] font-extrabold leading-none tracking-tight mb-5 transition-colors duration-300"
            style={{ color: "var(--hero-text)" }}>
            Everything you need<br />
            <em className="not-italic text-[var(--accent)]">to build & grow.</em>
          </h1>
          <p className="text-[17px] font-light leading-relaxed max-w-xl transition-colors duration-300"
      ...
      <section className="px-8 md:px-14 py-28 bg-[var(--bg-base)] transition-colors duration-500">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-[var(--border)] border border-[var(--border)] rounded-2xl overflow-hidden reveal transition-colors duration-500">
            {SERVICES.map((svc, i) => (
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              ...
              <div key={title} className={`reveal reveal-d${(i % 3) + 1} p-7 bg-[var(--bg-surface)] border border-[var(--border)] rounded-2xl hover:border-[var(--accent-pale2)] hover:shadow-[var(--shadow-md)] transition-all duration-300`}>
                <span className="text-3xl mb-4 block">{emoji}</span>
                <h3 className="font-syne text-base font-bold text-[var(--ink)] mb-2 tracking-tight">{title}</h3>
                <p className="text-[13px] font-light text-[var(--ink3)] leading-relaxed">{desc}</p>
              </div>
            ))}
          </div>
        </div>
