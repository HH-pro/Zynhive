...
// ════════════════════════════════════════════════════════════════════════════
// DESIGN TOKENS — deep navy + electric blue + warm gold accents
// Override your existing CSS vars with these in index.css / App.tsx
// ════════════════════════════════════════════════════════════════════════════
const THEME = `
...
function StatCard({ label, value, sub, color, icon }: {
  ...
  return (
    <div className="relative rounded-2xl p-5 overflow-hidden group transition-all duration-300 hover:-translate-y-0.5"
      ...
      <div className="absolute top-0 left-0 right-0 h-px" style={{ background:`linear-gradient(90deg, ${color}60, transparent 50%)` }}/>
...
function ProjectRow({ project, onEdit, onDelete }: {
  project: FirestoreProject; onEdit:(p:FirestoreProject)=>void; onDelete:(p:FirestoreProject)=>void;
}) {
  return (
    <div className="flex items-center gap-4 px-5 py-3.5 border-b last:border-0 group transition-all duration-200 cursor-default"
      ...
      <div className="w-10 h-10 rounded-xl flex-shrink-0 overflow-hidden"
        ...
        {project.imageUrl ? (
          <img src={getCloudinaryThumb(project.imageUrl,80,80)} alt={project.title} className="w-full h-full object-cover"/>
        ) : (
      ...
      <div className="flex items-center gap-1 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
        {[
          { title:"Edit",   color:"var(--accent)", bg:"var(--accent-pale)", fn:()=>onEdit(project),   icon:<svg width="11" height="11" viewBox="0 0 11 11" fill="none"><path d="M7.5 1.5l2 2L3 10H1V8L7.5 1.5z" stroke="currentColor" strokeWidth="1" strokeLinejoin="round"/></svg> },
          { title:"Delete", color:"#EF4444",       bg:"rgba(239,68,68,0.08)", fn:()=>onDelete(project), icon:<svg width="11" height="11" viewBox="0 0 11 11" fill="none"><path d="M1.5 3h8M4 3V1.5h3V3M3.5 3l.5 6.5M7.5 3l-.5 6.5" stroke="currentColor" strokeWidth="1" strokeLinecap="round"/></svg> },
        ].map(({ title, color, bg, fn, icon }) => (
...
function DeleteConfirm({ project, onConfirm, onCancel }: {
  project:FirestoreProject; onConfirm:()=>void; onCancel:()=>void;
}) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4"
      ...
      <div className="w-full max-w-[380px] rounded-2xl p-7 text-center"
        ...
        <div className="flex gap-3">
          <button onClick={onCancel} className="flex-1 py-2.5 rounded-xl text-[13px] transition-all"
            style={{ border:"1px solid var(--border2)", color:"var(--ink3)", cursor:"pointer", background:"transparent", fontFamily:"'DM Sans', sans-serif" }}
            onMouseEnter={(e)=>{ (e.currentTarget as HTMLElement).style.borderColor="var(--ink4)"; }}
          ...
          <button onClick={onConfirm} className="flex-1 py-2.5 rounded-xl text-[13px] font-semibold text-white transition-all"
            style={{ background:"linear-gradient(135deg, #dc2626, #EF4444)", cursor:"pointer", fontFamily:"'DM Sans', sans-serif" }}>
            Delete
          </button>
        </div>
      </div>
...
function AnalyticsTab({ projects }: { projects: FirestoreProject[] }) {
  ...
  const catColors: Record<string, string> = {
    "AI Development":      C.accent,
    "Web Application":     C.cyan,
    "Mobile App":          C.purple,
    "UI/UX Design":        C.gold,
    "Digital Marketing":   C.green,
    "Software Consulting": C.accent2,
  };

  const summaryStats = [
    { label:"Featured",    val:projects.filter((p)=>p.featured).length,       color:C.accent  },
    { label:"With Image",  val:projects.filter((p)=>p.imageUrl).length,        color:C.cyan    },
    { label:"Categories",  val:Object.keys(byCategory).length,                 color:C.purple  },
    { label:"Total Tags",  val:projects.reduce((a,p)=>a+p.tags.length,0),      color:C.gold    },
  ];
        ...
        {summaryStats.map(({ label, val, color }) => (
          <div key={label} className="rounded-2xl p-5 text-center relative overflow-hidden group transition-all duration-300 hover:-translate-y-0.5"
            ...
            <div className="font-mono text-[32px] font-black leading-none mb-2 relative z-10"
              style={{ color, textShadow:`0 0 24px ${color}40` }}>{val}</div>
      ...
      <div className="rounded-2xl p-6" style={{ background:"var(--bg-card)", border:"1px solid var(--border2)" }}>
        ...
        <div className="flex flex-col gap-2">
          {projects.filter((p)=>p.result).slice(0,5).map((p,i)=>(
            <div key={p.id} className="flex items-center gap-3 p-3 rounded-xl transition-all duration-150 cursor-default"
              style={{ background:"rgba(255,255,255,0.02)" }}
              onMouseEnter={(e)=>(e.currentTarget.style.background="rgba(255,255,255,0.04)")}
              onMouseLeave={(e)=>(e.currentTarget.style.background="rgba(255,255,255,0.02)")}>
              <span className="font-mono text-[10px] w-5 text-center flex-shrink-0"
                style={{ color:i===0?C.gold:i===1?C.ink3:i===2?C.accent:"var(--ink4)" }}>
                {i===0?"①":i===1?"②":i===2?"③":`${i+1}`}
              </span>
          ...
          {!projects.filter((p)=>p.result).length && (
...
const TABS: { id: Tab; label: string; icon: React.ReactNode; badge?: string }[] = [
  { id:"projects", label:"Projects",
    icon:<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><rect x="1" y="1" width="5" height="5" rx="1.2" stroke="currentColor" strokeWidth="1.1"/><rect x="8" y="1" width="5" height="5" rx="1.2" stroke="currentColor" strokeWidth="1.1"/><rect x="1" y="8" width="5" height="5" rx="1.2" stroke="currentColor" strokeWidth="1.1"/><rect x="8" y="8" width="5" height="5" rx="1.2" stroke="currentColor" strokeWidth="1.1"/></svg>
  },
  { id:"team", label:"Team",
    icon:<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><circle cx="5" cy="4.5" r="2.2" stroke="currentColor" strokeWidth="1.1"/><path d="M1 12c0-2.2 1.8-3.5 4-3.5s4 1.3 4 3.5" stroke="currentColor" strokeWidth="1.1" strokeLinecap="round"/><circle cx="11" cy="4" r="1.5" stroke="currentColor" strokeWidth="1.1"/><path d="M13 10.5c0-1.5-.9-2.5-2.2-2.5" stroke="currentColor" strokeWidth="1.1" strokeLinecap="round"/></svg>
  },
  { id:"leads", label:"Leads",
    icon:<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M1.5 3h11M1.5 7h7M1.5 11h5" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round"/><circle cx="11.5" cy="10" r="2" stroke="currentColor" strokeWidth="1.1"/></svg>
  },
  { id:"analytics", label:"Analytics",
    icon:<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M1 11L4 7.5l3 2.5 3-5 3-3" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/></svg>
  },
];
    ...
    <>
      ...
      <div className="min-h-screen flex" style={{ background:"var(--bg)", fontFamily:"'DM Sans', sans-serif", cursor:"default" }}>
        ...
        <aside className="flex flex-col flex-shrink-0 transition-all duration-300 relative"
          ...
          <div className="absolute top-0 right-0 bottom-0 w-px" style={{ background:"linear-gradient(180deg, transparent, rgba(59,130,246,0.3) 40%, rgba(6,182,212,0.2) 60%, transparent)" }}/>
          ...
          <div className="flex items-center h-16 px-4 gap-3 overflow-hidden flex-shrink-0"
            ...
            {sidebarOpen && (
              <div className="min-w-0">
                <div className="font-black text-[15px] tracking-tight leading-none" style={{ color:"var(--ink)" }}>
                  ZynHive<span style={{ color:"#3B82F6" }}>.</span>
                </div>
            ...
            <button onClick={()=>setSidebarOpen(!sidebarOpen)}
              ...
              <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                <path d={sidebarOpen?"M8 2L4 6l4 4":"M4 2l4 4-4 4"} stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </button>
          </div>
            ...
            {TABS.map((t) => {
              ...
              return (
                <button key={t.id} onClick={()=>setTab(t.id)} title={!sidebarOpen?t.label:undefined}
                  className="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 text-left w-full relative overflow-hidden"
                  style={{
                    cursor:"pointer",
                  ...
                  {isActive && <div className="absolute left-0 top-2 bottom-2 w-0.5 rounded-full" style={{ background:"#3B82F6", boxShadow:"0 0 8px #3B82F6" }}/>}
                  <span className="flex-shrink-0 ml-0.5">{t.icon}</span>
                  {sidebarOpen && <span className="font-medium text-[13px] whitespace-nowrap">{t.label}</span>}
                  {isActive && sidebarOpen && <div className="ml-auto w-1.5 h-1.5 rounded-full" style={{ background:"#60A5FA", boxShadow:"0 0 6px #60A5FA" }}/>}
                </button>
              );
            })}
          </nav>
          ...
          <header className="h-16 flex items-center justify-between px-6 flex-shrink-0"
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M3 2l4 3-4 3" stroke="var(--ink4)" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                <span className="font-semibold text-[14px]" style={{ color:"var(--ink)" }}>{tabMeta?.label}</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              {tab === "projects" && (
                <button onClick={()=>{ setEditProject(null); setFormOpen(true); }}
                  ...
                  onMouseEnter={(e)=>{ (e.currentTarget as HTMLElement).style.boxShadow="0 6px 24px rgba(59,130,246,0.4)"; }}
                  onMouseLeave={(e)=>{ (e.currentTarget as HTMLElement).style.boxShadow="0 4px 16px rgba(59,130,246,0.25)"; }}>
            ...
            {tab === "projects" && (
              <div className="p-6 flex flex-col gap-5">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  <StatCard label="Total" value={projects.length} sub="all categories" color={C.accent}
                  ...
                  <StatCard label="Featured" value={projects.filter((p)=>p.featured).length} sub="on homepage" color={C.cyan}
                    icon={<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M7 1l1.5 3.5 3.5.5-2.5 2.5.7 3.5L7 9.5l-3.2 1.5.7-3.5L2 5l3.5-.5L7 1z" stroke="currentColor" strokeWidth="1"/></svg>}/>
                  <StatCard label="With Images" value={projects.filter((p)=>p.imageUrl).length} sub="on Cloudinary" color={C.purple}
                    icon={<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M2 10l3-3.5 2.5 2.5 2-2.5L12 10H2z" stroke="currentColor" strokeWidth="1"/><rect x="1" y="2" width="12" height="10" rx="1.5" stroke="currentColor" strokeWidth="1"/></svg>}/>
                  <StatCard label="AI Projects" value={projects.filter((p)=>p.category==="AI Development").length} sub="ML & LLM builds" color={C.gold}
                    icon={<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><circle cx="5" cy="5" r="2" stroke="currentColor" strokeWidth="1"/><circle cx="9" cy="5" r="2" stroke="currentColor" strokeWidth="1"/><circle cx="7" cy="9" r="2" stroke="currentColor" strokeWidth="1"/></svg>}/>
                </div>
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  <div className="relative flex-1 min-w-[220px] max-w-sm">
                    ...
                    <input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Search projects…"
                      ...
                      style={{ background:"var(--bg-card)", border:"1px solid var(--border2)", color:"var(--ink)", outline:"none", cursor:"text", fontFamily:"'DM Sans', sans-serif" }}
                    ...
                    <div className="flex flex-col items-center justify-center py-20 gap-4">
                      <div className="relative w-9 h-9">
                        <div className="absolute inset-0 rounded-full border-2 border-t-transparent animate-spin"
                          style={{ borderColor:"var(--border2)", borderTopColor:"#3B82F6" }}/>
                        <div className="absolute inset-0 rounded-full border-2 border-b-transparent animate-spin"
                          style={{ borderColor:"transparent", borderBottomColor:"#06B6D4", animationDirection:"reverse", animationDuration:"1.5s" }}/>
                      </div>
                    ...
                    <div className="flex flex-col items-center justify-center py-20 gap-4">
                      <div className="w-14 h-14 rounded-2xl flex items-center justify-center"
                        ...
                        <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
                          ...
                          <path d="M8 11h6M11 8v6" stroke="#3B82F6" strokeWidth="1.5" strokeLinecap="round"/>
                        </svg>
                      </div>
                      <div className="text-center">
                        <p className="font-semibold text-[14px] mb-1" style={{ color:"var(--ink3)" }}>{search?"No results found":"No projects yet"}</p>
        ...
        {toast && (
          <div className="fixed bottom-6 right-6 z-[100] flex items-center gap-3 px-5 py-3.5 rounded-2xl"
            style={{
              background:   toast.type==="success" ? "var(--bg-card)" : "rgba(30,8,8,0.95)",
              border:       `1px solid ${toast.type==="success" ? "rgba(59,130,246,0.2)" : "rgba(239,68,68,0.25)"}`,
              color:        toast.type==="success" ? "var(--ink2)" : "#FCA5A5",
              boxShadow:    toast.type==="success" ? "0 8px 32px rgba(0,0,0,0.4), 0 0 0 1px rgba(59,130,246,0.08)" : "0 8px 32px rgba(0,0,0,0.5)",
            ...
            <div className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0"
              style={{
                background: toast.type==="success" ? "linear-gradient(135deg, #3B82F6, #06B6D4)" : "rgba(239,68,68,0.15)",
                color:      toast.type==="success" ? "white"                                      : "#F87171",
                boxShadow:  toast.type==="success" ? "0 0 10px rgba(59,130,246,0.4)" : "none",
              }}>
              {toast.type==="success" ? "✓" : "✕"}
            </div>
            {toast.msg}
