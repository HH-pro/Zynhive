...
interface Props {
  leads:   FirestoreLead[];
  onClose: () => void;
  onSend:  (leads: FirestoreLead[], emailType: FollowUpKey) => Promise<void>;
}
...
export function ComposeEmailModal({ leads, onClose, onSend }: Props) {
  ...
  useEffect(() => {
    ...
    async function gen() {
      setLoading(true);
      try {
        // mailHistory is optional on FirestoreLead
        const prev = (firstLead.mailHistory ?? [])
          .filter((m) => m.bodySnapshot)
          .map((m) => m.bodySnapshot!);
        const result = await generateEmail(firstLead, emailType, prev);
        if (!cancelled) {
        ...
        if (!cancelled) {
          ...
          // companyName replaces the old lead.name
          setSubject(`Follow-up for ${firstLead.companyName}`);
        }
      } finally {
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="font-mono text-[11px] mt-1" style={{ color: "var(--ink4)" }}>
              Sending to{" "}
              <strong style={{ color: "var(--ink)" }}>{leads.length}</strong>{" "}
              lead{leads.length !== 1 ? "s" : ""}{" · "}Each gets a personalized AI-generated email
            </p>
          </div>
        ...
        <div className="mb-5">
          ...
          <div className="flex flex-wrap gap-2 p-3 rounded-xl max-h-24 overflow-y-auto"
            style={{ background: "var(--bg-alt)", border: "1px solid var(--border)" }}>
            {leads.map((l) => (
              <span key={l.id}
                className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg font-mono text-[10px]"
                style={{ background: "var(--bg-surface)", border: "1px solid var(--border2)", color: "var(--ink3)" }}>
                {/* companyName replaces old lead.name */}
                <span className="font-semibold" style={{ color: "var(--ink)" }}>{l.companyName}</span>
        ...
        <div className="mb-5">
          <div className="flex items-center justify-between mb-2">
            <label className="font-mono text-[10px] font-semibold uppercase tracking-widest"
              style={{ color: "var(--ink4)" }}>
              {/* companyName replaces old lead.name */}
              AI Preview (for {firstLead?.companyName ?? "first lead"})
            </label>
            {loading && (
            ...
            {sending ? (
              ...
              <>
                <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
                  <path d="M1 6.5l4 2.5V12l2-2.5M1 6.5L12 1 8 12l-3-3"
