...
export function LeadAnalytics({ leads, stats }: Props) {
  ...
  // Email velocity — mailHistory is optional on FirestoreLead
  ...
  // Issue counts — all live under auditData (optional)
  const noChatbot        = leads.filter((l) => !l.auditData?.hasChatbot).length;
  const noQuickResponse  = leads.filter((l) => !l.auditData?.hasQuickResponse).length;
  const noLeadForm       = leads.filter((l) => !l.auditData?.hasLeadForm).length;
  const notMobile        = leads.filter((l) => !l.auditData?.hasMobileOptimized).length;
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <MiniStat label="Due Follow-ups"   value={stats.dueFollowUps}    color="var(--gold)"   icon="⏰" />
        <MiniStat label="Emails/Day (7d)"  value={velocity}              color="var(--purple)" icon="📈" />
      </div>
      ...
      <div className="rounded-2xl p-6"
        ...
        <div className="flex gap-1 h-10 rounded-xl overflow-hidden mb-4">
          {allStatuses.map(({ key, label }) => {
            ...
            const pct = Math.max((count / Math.max(stats.total, 1)) * 100, 5);
          ...
          {allStatuses.map(({ key, label }) => {
            ...
            return (
              <div key={key} className="flex items-center gap-2">
                ...
                <span className="font-mono text-[10px] font-bold" style={{ color: sc.text }}>{count}</span>
              </div>
            );
          })}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="rounded-2xl p-6"
          ...
          <div className="flex flex-col gap-4">
            {Object.entries(stats.bySource).sort(([, a], [, b]) => b - a).map(([src, cnt]) => {
        ...
        {/* Issue breakdown — all derived from auditData */}
        <div className="rounded-2xl p-6"
          style={{ background: "var(--bg-surface)", border: "1px solid var(--border2)" }}>
          <h3 className="font-display text-[15px] font-bold mb-5" style={{ color: "var(--ink)" }}>
          ...
          <div className="flex flex-col gap-4">
            <IssueBar label="No Chatbot"          count={noChatbot}       total={leads.length} color="var(--red)"    />
            <IssueBar label="No Quick Response"   count={noQuickResponse} total={leads.length} color="var(--gold)"   />
            <IssueBar label="No Lead Form"        count={noLeadForm}      total={leads.length} color="var(--purple)" />
            <IssueBar label="Not Mobile Optimized" count={notMobile}      total={leads.length} color="var(--accent)" />
          </div>
  ...
  return (
    <div className="rounded-2xl p-5 relative overflow-hidden"
      ...
      <div className="absolute inset-0 pointer-events-none"
        style={{ background: `radial-gradient(circle at 0% 0%, ${color}18, transparent 55%)` }} />
      <div className="flex items-center justify-between relative z-10 mb-2">
        <span className="font-mono text-[10px] tracking-[0.14em] uppercase font-semibold"
          style={{ color: "var(--ink4)" }}>{label}</span>
        <span className="text-base">{icon}</span>
      </div>
      <div className="font-display text-[28px] font-bold leading-none tracking-tight relative z-10"
        style={{
