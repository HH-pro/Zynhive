...
const getMailStatus     = (lead: FirestoreLead): string            => { const h = getMailHistory(lead); return h.length > 0 ? h[h.length - 1].type : "none"; };
...
function getNextEmailType(lead: FirestoreLead): FollowUpKey | null {
  ...
  if (idx >= 0 && idx < FOLLOW_UP_SEQUENCE.length - 1) return FOLLOW_UP_SEQUENCE[idx + 1].key;
  return null;
}
...
function Inp({ name, value, onChange, placeholder, type = "text", required }: {
  ...
  return (
    <input
      ...
      className="px-3 py-2.5 rounded-xl text-[13px] outline-none transition-colors duration-200 w-full"
      style={{ background: "var(--bg-alt)", color: "var(--ink)", border: "1px solid var(--border2)" }}
...
function Sel({ name, value, onChange, children }: {
  ...
  return (
    <select
      ...
      className="px-3 py-2.5 rounded-xl text-[13px] outline-none transition-colors duration-200 w-full"
      style={{ background: "var(--bg-alt)", color: "var(--ink)", border: "1px solid var(--border2)", cursor: "pointer" }}
    ...
    >
      ...
      >
        ...
        <div className="text-center">
          <p className="text-[14px] font-semibold" style={{ color: "var(--ink)", fontFamily: "'DM Sans', sans-serif" }}>
            AI Processing
          </p>
  ...
  return (
    <div className="flex flex-wrap gap-1">
      {issues.slice(0, 2).map((i) => (
        <span
          key={i}
          className="inline-flex items-center px-1.5 py-0.5 rounded-md font-mono text-[9px] font-bold"
...
function MailStatusBadge({ lead }: { lead: FirestoreLead }) {
  ...
  return (
    <span
      className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg font-mono text-[9px] font-bold"
...
// ══════════════════════════════════════════════════════════════════════════════
// LEAD FORM (slide-in panel)
// ══════════════════════════════════════════════════════════════════════════════
function LeadForm({
  lead, onClose, onSaved,
}: { lead?: FirestoreLead | null; onClose: () => void; onSaved: (msg: string) => void }) {
  const isEdit  = !!lead;
  ...
  return (
    ...
    >
      <div
        className="relative h-full w-full max-w-[640px] overflow-y-auto flex flex-col"
        style={{
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5 p-6 flex-1">
          ...
          <Field label="Company Logo">
            <div
              className="relative rounded-xl border-2 border-dashed flex items-center justify-center overflow-hidden transition-all duration-300"
              style={{
                height: form.imageUrl ? 120 : 80,
                borderColor: form.imageUrl ? "var(--accent)" : "var(--border2)",
                background: "var(--bg-surface)",
                cursor: "pointer",
              }}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <Field label="City">
              <Inp name="city" value={form.city} onChange={handleChange} placeholder="Lahore" />
            </Field>
            <Field label="Email">
              <Inp name="email" value={form.email} onChange={handleChange} placeholder="contact@co.com" type="email" />
            </Field>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <Field label="Phone">
              <Inp name="phone" value={form.phone} onChange={handleChange} placeholder="+92 300 0000000" />
            </Field>
            <Field label="WhatsApp">
              <Inp name="whatsapp" value={form.whatsapp} onChange={handleChange} placeholder="+92 300 0000000" />
            </Field>
            <Field label="Website">
              <Inp name="website" value={form.website} onChange={handleChange} placeholder="https://…" type="url" />
            </Field>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Social Links">
              <Inp name="socialLinks" value={form.socialLinks} onChange={handleChange} placeholder="LinkedIn / Instagram" />
            </Field>
            <Field label="Tags (comma separated)">
              <input
                value={tagInput} onChange={(e) => setTagInput(e.target.value)} placeholder="hot, enterprise"
                className="px-3 py-2.5 rounded-xl text-[13px] outline-none transition-colors duration-200 w-full"
                style={iBase}
                onFocus={(e) => { (e.target as HTMLInputElement).style.borderColor = "var(--accent)"; }}
                onBlur={(e)  => { (e.target as HTMLInputElement).style.borderColor = "var(--border2)"; }}
          ...
          <Field label="Notes / Proposal">
            <textarea
              name="proposal" value={form.proposal} onChange={handleChange} rows={4}
              placeholder="Notes, context, next steps…"
              className="px-3 py-2.5 rounded-xl text-[13px] outline-none resize-none transition-colors duration-200 leading-relaxed w-full"
              style={{ ...iBase, background: "var(--bg-alt)" }}
          ...
          {/* Footer buttons */}
          <div
            className="flex gap-3 pt-2 sticky bottom-0 -mx-6 px-6 pb-6 mt-auto"
          ...
          >
            <button
              ...
              style={{
                ...
                cursor: "pointer", background: "transparent", fontFamily: "'DM Sans', sans-serif",
              }}
              onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = "var(--bg-alt)"; }}
              onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = "transparent"; }}
            >
            ...
            <button
              ...
              className="flex-1 py-3 rounded-xl text-[14px] font-semibold text-white disabled:opacity-60 transition-all"
              style={{
...
function DeleteConfirm({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-[380px] rounded-2xl p-7 text-center"
        style={{
          background: "var(--bg-surface)",
          border: "1px solid var(--border2)",
      ...
      >
        ...
        <p
          ...
          style={{ color: "var(--ink4)", fontFamily: "'DM Sans', sans-serif" }}
        >
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            style={{ background: "linear-gradient(135deg, #e53535, #EF4444)", cursor: "pointer" }}
          >
            Delete
          </button>
        </div>
...
// ══════════════════════════════════════════════════════════════════════════════
function LeadModal({
  lead, onClose, onEdit, onAudit, onSendEmail, aiLoading,
}: {
  lead: FirestoreLead;
  onClose: () => void;
  onEdit: () => void;
  ...
  async function handleOpenCompose(emailType: FollowUpKey) {
    setPendingType(emailType); setComposing(true);
    setGenLoading(true); setEditSubject(""); setEditBody("");
    try {
      ...
      setEditSubject(res.subject); setEditBody(res.body);
    } catch {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-[680px] max-h-[90vh] overflow-y-auto rounded-2xl"
        style={{
          background: "var(--bg-surface)",
          border: "1px solid var(--border2)",
      ...
      >
        ...
        <div className="flex items-start justify-between p-6 pb-4" style={{ borderBottom: "1px solid var(--border)" }}>
          <div className="flex items-start gap-4 flex-1 min-w-0">
            {lead.imageUrl ? (
              <img src={lead.imageUrl} alt={lead.companyName}
                className="w-12 h-12 rounded-xl object-contain flex-shrink-0"
                style={{ background: "var(--bg-panel)", border: "1px solid var(--border2)", padding: 4 }}
              />
            ) : (
              <div
                className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 font-bold text-xl text-white"
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                <span
                  className="px-2 py-0.5 rounded-lg font-mono text-[9px] font-bold tracking-widest"
                  style={{ background: st.bg, color: st.color }}
                >
                  {st.label}
                </span>
                <span
                  className="px-2 py-0.5 rounded-lg font-mono text-[9px] font-bold"
                ...
                <span className="font-mono text-[10px]" style={{ color: "var(--ink4)" }}>
                  Score:{" "}
                  <strong style={{ color: score > 70 ? "#10B981" : score > 40 ? "#F59E0B" : "#EF4444" }}>
                    {score}
                  </strong>
                </span>
              </div>
        ...
        <div className="flex gap-1 px-6 pt-4">
          {(["details", "emails", "notes"] as const).map((t) => (
            <button
              ...
              style={{
                background: tab === t ? "var(--accent-pale)" : "transparent",
                color: tab === t ? "var(--accent)" : "var(--ink4)",
                border: tab === t ? "1px solid var(--accent-pale2)" : "1px solid transparent",
                cursor: "pointer",
              }}
            >
            ...
            <div className="flex flex-col gap-4">
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  >
                    <span className="font-mono text-[9px] tracking-[0.12em] uppercase" style={{ color: "var(--ink4)" }}>
                      {label}
                    </span>
                    {href
                      ? <a href={href} target="_blank" rel="noopener noreferrer" className="text-[13px] truncate" style={{ color: "var(--accent)", fontFamily: "'DM Sans', sans-serif" }}>{val}</a>
                      : <span className="text-[13px] truncate" style={{ color: "var(--ink)", fontFamily: "'DM Sans', sans-serif" }}>{val}</span>
                    }
                  </div>
                ))}
              </div>
              ...
              <div className="p-4 rounded-xl" style={{ background: "var(--bg-alt)", border: "1px solid var(--border)" }}>
                <div className="flex items-center justify-between mb-3">
                  <span className="font-mono text-[10px] tracking-[0.12em] uppercase" style={{ color: "var(--ink4)" }}>
                    🤖 AI Website Audit
                  </span>
                  {lead.website && (
                    <button
                      onClick={() => onAudit(lead.id!)} disabled={aiLoading}
                      className="font-mono text-[10px] px-3 py-1.5 rounded-lg font-semibold transition-all disabled:opacity-50"
                      style={{
                        background: "var(--accent-pale)", color: "var(--accent)",
                        cursor: aiLoading ? "wait" : "pointer", border: "1px solid var(--accent-pale2)",
                      }}
                    >
                ...
                {audit ? (
                  <div className="flex flex-col gap-2">
                    ...
                    <p className="text-[12px] leading-relaxed" style={{ color: "var(--ink3)", fontFamily: "'DM Sans', sans-serif" }}>{audit.summary}</p>
                    <span className="font-mono text-[10px]" style={{ color: "var(--ink4)" }}>
                      Score:{" "}
                      <strong style={{ color: audit.score > 70 ? "#10B981" : audit.score > 40 ? "#F59E0B" : "#EF4444" }}>
                        {audit.score}/100
                      </strong>
                    </span>
                  </div>
                ) : (
                  <p className="text-[12px]" style={{ color: "var(--ink4)", fontFamily: "'DM Sans', sans-serif" }}>
              ...
              {lead.proposal && (
                <div className="p-4 rounded-xl" style={{ background: "var(--bg-alt)", border: "1px solid var(--border)" }}>
                  ...
                  <p className="text-[13px] leading-relaxed" style={{ color: "var(--ink3)", fontFamily: "'DM Sans', sans-serif" }}>{lead.proposal}</p>
                </div>
              )}
            </div>
          )}
          ...
          {tab === "emails" && (
            <div className="flex flex-col gap-4">
              {composing ? (
                <div className="flex flex-col gap-3 p-4 rounded-2xl" style={{ background: "var(--bg-alt)", border: "1px solid var(--accent-pale2)" }}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="text-[14px] font-bold" style={{ color: "var(--ink)", fontFamily: "'DM Sans', sans-serif" }}>
                    ...
                    <button onClick={handleCancelCompose} className="font-mono text-[10px] px-2.5 py-1 rounded-lg"
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <input value={editSubject} onChange={(e) => setEditSubject(e.target.value)} disabled={genLoading}
                      ...
                      className="px-3 py-2.5 rounded-xl text-[13px] outline-none border w-full transition-colors"
                      style={{ background: "var(--bg-surface)", color: "var(--ink)", borderColor: "var(--border2)", opacity: genLoading ? 0.5 : 1, fontFamily: "'DM Sans', sans-serif" }}
                      onFocus={(e) => { (e.target as HTMLInputElement).style.borderColor = "var(--accent)"; }}
                      onBlur={(e)  => { (e.target as HTMLInputElement).style.borderColor = "var(--border2)"; }}
                    />
                  </div>
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <textarea value={editBody} onChange={(e) => setEditBody(e.target.value)} disabled={genLoading} rows={8}
                      ...
                      className="px-3 py-2.5 rounded-xl text-[13px] outline-none border w-full resize-none leading-relaxed transition-colors"
                      style={{ background: "var(--bg-surface)", color: "var(--ink)", borderColor: "var(--border2)", opacity: genLoading ? 0.5 : 1, fontFamily: "'DM Sans', sans-serif" }}
                      onFocus={(e) => { (e.target as HTMLTextAreaElement).style.borderColor = "var(--accent)"; }}
                      onBlur={(e)  => { (e.target as HTMLTextAreaElement).style.borderColor = "var(--border2)"; }}
                    />
                  ...
                  <div className="flex gap-2 pt-1">
                    <button onClick={() => pendingType && handleOpenCompose(pendingType)} disabled={genLoading || aiLoading}
                    ...
                    <button onClick={handleConfirmSend} disabled={genLoading || aiLoading || !editSubject || !editBody}
                ...
                nextEmail ? (
                  <div className="flex flex-col gap-2">
                    <button onClick={() => onSendEmail(lead.id!, nextEmail)} disabled={aiLoading}
                      ...
                      ⚡ Quick Send — {FOLLOW_UP_SEQUENCE.find((s) => s.key === nextEmail)?.label}
                    </button>
              ...
              {!composing && (
                mailHist.length === 0 ? (
                  <div className="text-center py-10">
                    <p className="text-[14px]" style={{ color: "var(--ink4)", fontFamily: "'DM Sans', sans-serif" }}>No emails sent yet</p>
                    ...
                    {mailHist.map((mail, i) => (
                      <div key={i} className="relative pb-5 last:pb-0">
                        ...
                        <div className="flex items-center gap-3 mb-1">
                          <span className="text-[13px] font-semibold" style={{ color: "var(--ink)", fontFamily: "'DM Sans', sans-serif" }}>
                        ...
                        {mail.subject && (
                          <p className="font-mono text-[11px] mb-1" style={{ color: "var(--ink3)" }}>Subject: {mail.subject}</p>
                        )}
                        {mail.bodySnapshot && (
                          <details className="mt-1">
                            <summary className="font-mono text-[10px] cursor-pointer" style={{ color: "var(--accent)" }}>View email body</summary>
                            <div className="mt-1.5 p-3 rounded-lg text-[11px] leading-relaxed whitespace-pre-wrap max-h-40 overflow-y-auto"
                              style={{ color: "var(--ink3)", background: "var(--bg-alt)", border: "1px solid var(--border)", fontFamily: "'DM Sans', sans-serif" }}>
                              {mail.bodySnapshot}
                            </div>
                          </details>
                        )}
                  ...
                  {lead.notes.map((n, i) => (
                    <div key={i} className="p-4 rounded-xl" style={{ background: "var(--bg-alt)", border: "1px solid var(--border)" }}>
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-[13px] font-bold" style={{ color: "var(--ink)", fontFamily: "'DM Sans', sans-serif" }}>{n.employeeName}</span>
                        <span className="font-mono text-[10px]" style={{ color: "var(--ink4)" }}>{n.date ? new Date(n.date).toLocaleDateString() : ""}</span>
                      </div>
                      <p className="text-[13px]" style={{ color: "var(--ink3)", fontFamily: "'DM Sans', sans-serif" }}>{n.text}</p>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-center py-10 text-[14px]" style={{ color: "var(--ink4)", fontFamily: "'DM Sans', sans-serif" }}>No notes yet</p>
              )}
            </div>
          )}
        </div>
        ...
        <div className="flex gap-3 px-6 pb-6 pt-5" style={{ borderTop: "1px solid var(--border)" }}>
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl text-[13px] font-medium border transition-all"
            style={{ borderColor: "var(--border2)", color: "var(--ink3)", cursor: "pointer", background: "transparent", fontFamily: "'DM Sans', sans-serif" }}>
            Close
          </button>
          <button onClick={onEdit} className="flex-1 py-2.5 rounded-xl text-[13px] font-semibold text-white"
            style={{ background: "linear-gradient(135deg, var(--accent), var(--cyan))", cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>
            Edit Lead →
          </button>
        </div>
      </div>
...
// ══════════════════════════════════════════════════════════════════════════════
// AI SEARCH MODAL — Google Maps Scraper
// ══════════════════════════════════════════════════════════════════════════════
function AiSearchModal({
  onClose, onImport,
}: { onClose: () => void; onImport: (leads: Partial<FirestoreLead>[]) => Promise<void> }) {
  ...
  async function handleImport() {
    ...
    await onImport(sel.map((r) => ({
      ...EMPTY_FORM,
      companyName: r.companyName, email: r.email, phone: r.phone,
      website: r.website, country: r.country, city: r.city,
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-[620px] max-h-[85vh] overflow-y-auto rounded-2xl p-6"
        style={{
          background: "var(--bg-surface)",
          border: "1px solid var(--border2)",
          animation: "fadeScaleIn .25s cubic-bezier(0.16,1,0.3,1) both",
      ...
      >
        <div className="flex items-center justify-between mb-5">
          <div>
            <h2 className="text-[17px] font-bold" style={{ color: "var(--ink)", fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          <div className="flex flex-col gap-1">
            ...
            <input value={keyword} onChange={(e) => setKeyword(e.target.value)} placeholder="e.g. Real Estate Agency"
              ...
              style={{ background: "var(--bg-alt)", color: "var(--ink)", fontFamily: "'DM Sans', sans-serif" }}
        ...
        {!HASDATA_KEY && !OUTSCRAPER_KEY && (
          <div className="mb-4 px-3 py-2.5 rounded-xl border text-[11px] font-mono"
            ...
            ⚠️ Add free API key to .env for real data:{" "}
            <span style={{ color: "var(--accent)" }}>hasdata.com</span> → VITE_HASDATA_API_KEY=xxx
          </div>
        )}
                ...
                <div key={i} className="flex items-start gap-3 px-4 py-3 border-b last:border-0 transition-colors"
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className="text-[13px] font-semibold truncate" style={{ color: "var(--ink)", fontFamily: "'DM Sans', sans-serif" }}>{r.companyName}</span>
                      {r.rating > 0 && (
...
// ══════════════════════════════════════════════════════════════════════════════
// BATCH EMAIL COMPOSE MODAL
// ══════════════════════════════════════════════════════════════════════════════
function ComposeModal({
  leads, onClose, onSend,
}: { leads: FirestoreLead[]; onClose: () => void; onSend: (leads: FirestoreLead[], type: FollowUpKey) => Promise<void> }) {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-[580px] max-h-[85vh] overflow-y-auto rounded-2xl p-6"
        style={{
          background: "var(--bg-surface)",
          border: "1px solid var(--border2)",
          animation: "fadeScaleIn .25s cubic-bezier(0.16,1,0.3,1) both",
      ...
      >
        <div className="flex items-center justify-between mb-5">
          <div>
            <h2 className="text-[17px] font-bold" style={{ color: "var(--ink)", fontFamily: "'DM Sans', sans-serif" }}>📧 AI Email Composer</h2>
        ...
        <div className="flex flex-wrap gap-2 mb-5">
          {FOLLOW_UP_SEQUENCE.map((step) => (
            <button key={step.key} onClick={() => setEmailType(step.key)}
              ...
              style={{
                cursor: "pointer",
                background: emailType === step.key ? "linear-gradient(135deg, var(--accent), var(--cyan))" : "var(--bg-alt)",
        ...
        <div className="mb-5">
          ...
          <div className="p-4 rounded-b-xl text-[12px] leading-relaxed whitespace-pre-wrap max-h-48 overflow-y-auto"
            style={{ background: "var(--bg-alt)", border: "1px solid var(--border2)", color: loading ? "var(--ink4)" : "var(--ink3)", fontFamily: "'DM Sans', sans-serif" }}>
            {loading ? "✨ AI is crafting a personalised email..." : preview}
          </div>
          <p className="font-mono text-[9px] mt-1.5" style={{ color: "var(--ink4)" }}>
            Each lead gets a unique AI-personalised version
          </p>
        </div>

        <div className="flex justify-end gap-3">
          <button onClick={onClose} className="px-5 py-2.5 rounded-xl text-[13px] font-medium border transition-all"
            style={{ borderColor: "var(--border2)", color: "var(--ink3)", cursor: "pointer", background: "transparent", fontFamily: "'DM Sans', sans-serif" }}>
            Cancel
          </button>
          <button onClick={handleSend} disabled={sending || loading}
            ...
            style={{ background: "linear-gradient(135deg, var(--accent), var(--cyan))", cursor: sending ? "wait" : "pointer" }}>
            {sending
...
export function LeadTab({ showToast }: Props) {
  ...
  // AI: generate + send single email
  async function handleSendEmail(
    leadId: string, emailType: FollowUpKey,
    ...
    try {
      ...
      const sendResult = await sendEmail({ to: lead.email, subject: subject!, body: body! });
      ...
      for (let i = 0; i < targetLeads.length; i++) {
        ...
        try {
          ...
          await updateLead(lead.id!, {
            mailHistory: history, lastContacted: today(),
            followUpCount: (lead.followUpCount ?? 0) + 1,
            status: lead.status === "new" ? "contacted" : lead.status,
          } as any);
    ...
    <div className="p-6 flex flex-col gap-5">
        ...
        <div className="flex rounded-xl p-1" style={{ background: "var(--bg-alt)", border: "1px solid var(--border2)" }}>
          {([
            ...
            <button
              ...
              style={{
                cursor: "pointer",
                background: subTab === id ? "linear-gradient(135deg, var(--accent), var(--cyan))" : "transparent",
                color: subTab === id ? "white" : "var(--ink4)",
              }}
            >
              {label}
              {count !== null && count > 0 && (
                <span
                  ...
                  style={{
                    background: subTab === id ? "rgba(255,255,255,0.2)" : "var(--bg-surface)",
                    color: subTab === id ? "white" : "var(--ink4)",
                  }}
                >
                  {count}
        ...
        <>
            ...
            {([
              ...
              >
                <div
                  ...
                  style={{ background: `radial-gradient(circle at 0% 0%, ${color}14, transparent 55%)` }}
                />
                <span className="font-mono text-[9px] tracking-[0.14em] uppercase relative z-10" style={{ color: "var(--ink4)" }}>
                  {label}
                </span>
                <span className="text-[24px] font-bold leading-none relative z-10" style={{ color: "var(--ink)", fontFamily: "'DM Sans', sans-serif" }}>
                  {val}
                </span>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input
                ...
                style={{ background: "var(--bg-surface)", color: "var(--ink)", cursor: "text", fontFamily: "'DM Sans', sans-serif" }}
                onFocus={(e) => { (e.target as HTMLInputElement).style.borderColor = "var(--accent)"; }}
                onBlur={(e)  => { (e.target as HTMLInputElement).style.borderColor = "var(--border2)"; }}
              />
            </div>
            ...
            <div className="font-mono text-[11px] ml-auto" style={{ color: "var(--ink4)" }}>
          ...
          <div className="rounded-2xl overflow-hidden" style={{ background: "var(--bg-surface)", border: "1px solid var(--border2)" }}>
            ...
            <div
              ...
              style={{
                gridTemplateColumns: "36px 2fr 1fr 1fr 100px 100px 80px",
                background: "var(--bg-alt)",
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-20">
                <p className="text-[14px] font-semibold" style={{ color: "var(--ink3)", fontFamily: "'DM Sans', sans-serif" }}>
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      cursor: "default",
                      background: isSel ? "var(--accent-pale)" : "transparent",
                    }}
                    onMouseEnter={(e) => {
                      if (!isSel) (e.currentTarget as HTMLElement).style.background = "var(--bg-alt)";
                    }}
                    onMouseLeave={(e) => {
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 min-w-0 cursor-pointer" onClick={() => setViewLead(lead)}>
                      ...
                      <div className="min-w-0">
                        <div className="text-[13px] font-semibold truncate" style={{ color: "var(--ink)", fontFamily: "'DM Sans', sans-serif" }}>
                          {lead.companyName}
                        </div>
                        <div className="font-mono text-[10px] truncate" style={{ color: "var(--ink4)" }}>
                      ...
                      {/* Score badge */}
                      <span
                        className="flex-shrink-0 w-6 h-6 rounded-full text-[9px] font-bold flex items-center justify-center ml-1"
                        style={{
                          background:
                            score > 70
                    ...
                    {/* Status */}
                    <div className="flex items-center">
                      <span className="px-2 py-0.5 rounded-lg font-mono text-[9px] font-bold tracking-widest"
                        style={{ background: st.bg, color: st.color }}>
                        {st.label}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                      {[
                        ...
                        <button
                          ...
                          style={{ color: "var(--ink4)", cursor: "pointer" }}
                          onMouseEnter={(e) => {
                            (e.currentTarget as HTMLElement).style.background = danger ? "rgba(239,68,68,0.1)" : "var(--accent-pale)";
                            (e.currentTarget as HTMLElement).style.color = danger ? "#EF4444" : "var(--accent)";
                          }}
                          onMouseLeave={(e) => {
                            (e.currentTarget as HTMLElement).style.background = "transparent";
                            (e.currentTarget as HTMLElement).style.color = "var(--ink4)";
                          }}
                        >
                          {icon}
                        </button>
      ...
      {subTab === "followups" && (
        <>
            ...
            {FOLLOW_UP_SEQUENCE.map((step) => {
              ...
              return (
                <div key={step.key} className="rounded-xl p-4 text-center" style={{ background: "var(--bg-surface)", border: "1px solid var(--border2)" }}>
                  <div className="text-[22px] font-bold leading-none mb-1" style={{ color: sc.text, fontFamily: "'DM Sans', sans-serif" }}>{count}</div>
          ...
          <div className="flex items-center justify-between">
            <h3 className="text-[15px] font-bold" style={{ color: "var(--ink)", fontFamily: "'DM Sans', sans-serif" }}>
              ⏰ Due Follow-ups ({dueFollowUps.length})
            </h3>
            {dueFollowUps.length > 0 && (
              <button
                onClick={async () => { for (const l of dueFollowUps) { const d = getDueFollowUp(l); if (d) await handleSendEmail(l.id!, d.key); } }}
                disabled={!!aiLoading}
                className="flex items-center gap-2 px-4 py-2 rounded-xl text-[12px] font-semibold text-white disabled:opacity-50"
                style={{ background: "linear-gradient(135deg, var(--accent), var(--cyan))", cursor: aiLoading ? "wait" : "pointer" }}
              >
                Send All Follow-ups
              </button>
            )}
          ...
          {dueFollowUps.length === 0 ? (
            <div className="text-center py-16">
              <p className="text-[14px] font-semibold" style={{ color: "var(--ink3)", fontFamily: "'DM Sans', sans-serif" }}>All caught up!</p>
            ...
            <div className="rounded-2xl overflow-hidden" style={{ background: "var(--bg-surface)", border: "1px solid var(--border2)" }}>
              {dueFollowUps.map((lead) => {
                const due  = getDueFollowUp(lead)!;
                const last = getMailHistory(lead).at(-1)!;
                const days = daysAgo(last.date);
                ...
                return (
                  <div key={lead.id} className="flex items-center gap-4 px-5 py-3.5 border-b last:border-0" style={{ borderColor: "var(--border)" }}>
                    <div className="flex-1 min-w-0">
                      <div className="text-[13px] font-semibold" style={{ color: "var(--ink)", fontFamily: "'DM Sans', sans-serif" }}>{lead.companyName}</div>
                    ...
                    <div className="text-center">
                      <div className="font-mono text-[11px]" style={{ color: "var(--ink3)" }}>
                        {FOLLOW_UP_SEQUENCE.find((s) => s.key === last.type)?.label}
                      </div>
                    ...
                    <button
                      onClick={() => handleSendEmail(lead.id!, due.key)} disabled={!!aiLoading}
          ...
          {/* New leads without initial email */}
          {leads.filter((l) => l.status === "new" && getMailHistory(l).length === 0).length > 0 && (
            <>
              <h3 className="text-[15px] font-bold mt-2" style={{ color: "var(--ink)", fontFamily: "'DM Sans', sans-serif" }}>
                📋 New — Review & Send Initial Email
              </h3>
              <div className="rounded-2xl overflow-hidden" style={{ background: "var(--bg-surface)", border: "1px solid var(--border2)" }}>
                {leads.filter((l) => l.status === "new" && getMailHistory(l).length === 0).map((lead) => (
                  <div key={lead.id} className="flex items-center gap-4 px-5 py-3.5 border-b last:border-0" style={{ borderColor: "var(--border)" }}>
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => setViewLead(lead)} className="px-3 py-1.5 rounded-lg text-[11px] font-semibold"
                        style={{ border: "1px solid var(--border2)", color: "var(--ink3)", cursor: "pointer", background: "transparent" }}>
                        👁 Review
                      </button>
                      {lead.website && (
                        <button onClick={() => handleAudit(lead.id!)} disabled={!!aiLoading} className="px-3 py-1.5 rounded-lg text-[11px] font-semibold disabled:opacity-50"
                          style={{ border: "1px solid var(--border2)", color: "var(--ink3)", cursor: "pointer", background: "transparent" }}>
                          🤖 Audit
                        </button>
                      )}
                      <button onClick={() => handleSendEmail(lead.id!, "initial")} disabled={!!aiLoading}
                        className="px-3 py-1.5 rounded-lg text-[11px] font-semibold text-white disabled:opacity-50"
                        style={{ background: "linear-gradient(135deg, var(--accent), var(--cyan))", cursor: aiLoading ? "wait" : "pointer" }}>
                        📧 Send Initial
                      </button>
      ...
      {subTab === "crm-analytics" && (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {([
              ...
              { label: "Due Follow-ups",  val: stats.dueFollowUps, color: "#F59E0B",      icon: "⏰" },
              {
                ...
                val: stats.total > 0 ? `${Math.round((stats.closed / stats.total) * 100)}%` : "0%",
                color: "#10B981", icon: "📊",
              },
              ...
              >
                ...
                <div
                  className="text-[28px] font-bold leading-none relative z-10"
                  style={{
          ...
          {/* Email pipeline visual */}
          <div className="rounded-2xl p-6" style={{ background: "var(--bg-surface)", border: "1px solid var(--border2)" }}>
            <h3 className="text-[15px] font-bold mb-5" style={{ color: "var(--ink)", fontFamily: "'DM Sans', sans-serif" }}>Email Pipeline</h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            {/* Lead sources */}
            <div className="rounded-2xl p-6" style={{ background: "var(--bg-surface)", border: "1px solid var(--border2)" }}>
              <h3 className="text-[15px] font-bold mb-5" style={{ color: "var(--ink)", fontFamily: "'DM Sans', sans-serif" }}>Lead Sources</h3>
              {Object.entries(
                leads.reduce<Record<string, number>>((a, l) => { const s = l.leadSource || "Unknown"; a[s] = (a[s] || 0) + 1; return a; }, {}),
              ).sort(([, a], [, b]) => b - a).map(([src, cnt]) => {
            ...
            {/* Status distribution */}
            <div className="rounded-2xl p-6" style={{ background: "var(--bg-surface)", border: "1px solid var(--border2)" }}>
              <h3 className="text-[15px] font-bold mb-5" style={{ color: "var(--ink)", fontFamily: "'DM Sans', sans-serif" }}>Lead Status</h3>
      ...
      {/* ── OVERLAYS ───────────────────────────────────────────────────────── */}
      {formOpen && (
        <LeadForm lead={editLead} onClose={() => setFormOpen(false)} onSaved={(msg) => { load(); showToast(msg); }} />
      )}
      {resolvedView && (
        <LeadModal
          lead={resolvedView}
          onClose={() => setViewLead(null)}
          onEdit={() => { setEditLead(resolvedView); setViewLead(null); setFormOpen(true); }}
          onAudit={handleAudit}
      ...
      {deleteTgt && (
        <DeleteConfirm name={deleteTgt.companyName} onConfirm={handleDelete} onCancel={() => setDeleteTgt(null)} />
      )}
      {showCompose && selectedLeads.length > 0 && (
        <ComposeModal leads={selectedLeads} onClose={() => setShowCompose(false)} onSend={handleBatchSend} />
      )}
      {showAiSearch && (
        <AiSearchModal onClose={() => setShowAiSearch(false)} onImport={handleAiImport} />
      )}
    </div>
  );
}