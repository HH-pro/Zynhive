...
// Complete unified CRM tab — AI audit + email generation + real email sending
import {
...
const FOLLOW_UP_SEQUENCE = [
  { key: "initial"    as FollowUpKey, label: "Initial Email",       daysAfter: 0 },
  { key: "followup-1" as FollowUpKey, label: "Follow-up 1",         daysAfter: 5 },
  { key: "followup-2" as FollowUpKey, label: "Follow-up 2",         daysAfter: 7 },
  { key: "followup-3" as FollowUpKey, label: "Follow-up 3",         daysAfter: 5 },
  { key: "followup-4" as FollowUpKey, label: "Follow-up 4",         daysAfter: 7 },
  { key: "followup-5" as FollowUpKey, label: "Follow-up 5 (Final)", daysAfter: 7 },
];
...
function calcScore(lead: Partial<FirestoreLead>) {
  let s = 0;
...
const getMailHistory = (lead: FirestoreLead): MailEntry[] => (lead as any).mailHistory ?? [];
const getMailStatus  = (lead: FirestoreLead): string => {
  const h = getMailHistory(lead); return h.length > 0 ? h[h.length-1].type : "none";
};
const getAuditData   = (lead: FirestoreLead): AuditResult | null => (lead as any).auditData ?? null;

function getDueFollowUp(lead: FirestoreLead): typeof FOLLOW_UP_SEQUENCE[number] | null {
  ...
  if (idx >= 0 && idx < FOLLOW_UP_SEQUENCE.length - 1) {
    ...
    if (days >= next.daysAfter) return next;
  }
  return null;
}
...
function getNextEmailType(lead: FirestoreLead): FollowUpKey | null {
  ...
  const last = hist[hist.length-1].type;
...
function Label({ children }: { children: React.ReactNode }) {
  return <span className="font-mono text-[10px] tracking-[0.12em] uppercase" style={{ color:"var(--ink3)" }}>{children}</span>;
}
...
function Sel({ name, value, onChange, children }: {
  name:string; value:string; onChange:(e:ChangeEvent<HTMLSelectElement>)=>void; children:React.ReactNode;
}) {
  return (
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center" style={{ background:"rgba(0,0,0,0.6)", backdropFilter:"blur(6px)" }}>
      <div className="flex flex-col items-center gap-4 p-8 rounded-2xl"
        ...
        <div className="relative w-12 h-12">
          <div className="absolute inset-0 rounded-full border-2 border-t-transparent animate-spin" style={{ borderColor:"var(--border2)", borderTopColor:"var(--accent)" }}/>
          <div className="absolute inset-2 rounded-full flex items-center justify-center"><span className="text-lg">🤖</span></div>
        </div>
...
function AuditTags({ audit }: { audit: AuditResult | null }) {
  ...
  if (!issues.length)
    return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md font-mono text-[9px] font-bold" style={{ background:"rgba(34,197,94,0.08)", color:"#86efac", border:"1px solid rgba(34,197,94,0.2)" }}>✓ All Good</span>;
  return (
    <div className="flex flex-wrap gap-1">
      {issues.slice(0,2).map((i) => <span key={i} className="inline-flex items-center px-1.5 py-0.5 rounded-md font-mono text-[9px] font-bold" style={{ background:"rgba(239,68,68,0.08)", color:"#fca5a5", border:"1px solid rgba(239,68,68,0.2)" }}>{i}</span>)}
      {issues.length > 2 && <span className="font-mono text-[9px] px-1 py-0.5 rounded" style={{ color:"var(--ink4)", background:"var(--bg-alt)" }}>+{issues.length-2}</span>}
    </div>
  );
}
...
function MailStatusBadge({ lead }: { lead:FirestoreLead }) {
  ...
  return (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg font-mono text-[9px] font-bold" style={{ background:sc.bg, color:sc.text, border:`1px solid ${sc.border}` }}>
...
// LEAD FORM
// ══════════════════════════════════════════════════════════════════════════════
function LeadForm({ lead, onClose, onSaved }: { lead?:FirestoreLead|null; onClose:()=>void; onSaved:(msg:string)=>void }) {
  const isEdit = !!lead;
  ...
  function handleChange(e: ChangeEvent<HTMLInputElement|HTMLSelectElement|HTMLTextAreaElement>) {
    ...
    field(name, type === "checkbox" ? (e.target as HTMLInputElement).checked : value);
  }
  async function handleImage(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      field("imageUrl", res.secure_url); field("imagePublicId", res.public_id);
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      const data = { ...form, tags, leadScore: calcScore({ ...form, tags }) };
      if (isEdit && lead?.id) { await updateLead(lead.id, data); onSaved("Lead updated!"); }
      else                    { await createLead(data);          onSaved("Lead created!"); }
      onClose();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-end" style={{ background:"rgba(0,0,0,0.65)", backdropFilter:"blur(6px)" }} onClick={(e)=>e.target===e.currentTarget&&onClose()}>
      <div className="relative h-full w-full max-w-[640px] overflow-y-auto flex flex-col" style={{ background:"var(--bg-panel)", borderLeft:"1px solid var(--border2)", animation:"slideInRight .3s cubic-bezier(0.16,1,0.3,1) both" }}>
        <div className="flex items-center justify-between px-6 py-5 sticky top-0 z-10" style={{ background:"var(--bg-panel)", borderBottom:"1px solid var(--border2)" }}>
          <div>
            <h2 className="font-display text-[17px] font-bold tracking-tight" style={{ color:"var(--ink)" }}>{isEdit ? "Edit Lead" : "Add New Lead"}</h2>
            <p className="font-mono text-[10px] mt-0.5" style={{ color:"var(--ink4)" }}>{isEdit ? `ID: ${lead?.id?.slice(0,8)}…` : "New entry → Firestore"}</p>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ color:"var(--ink4)", cursor:"pointer" }}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5 p-6 flex-1">
          <Field label="Company Logo">
            <div className="relative rounded-xl border-2 border-dashed flex items-center justify-center overflow-hidden transition-all duration-300"
              ...
              {form.imageUrl ? (
                ...
                <div className="absolute inset-0 flex items-center justify-center opacity-0 transition-opacity duration-200" style={{ background:"rgba(0,0,0,0.5)" }}
                ...
                <div className="flex flex-col items-center gap-2 w-full px-8">
                  <div className="w-full rounded-full h-1.5 overflow-hidden" style={{ background:"var(--border2)" }}>
                    <div className="h-full rounded-full transition-all duration-200" style={{ width:`${uploadPct}%`, background:"linear-gradient(90deg, var(--accent), var(--cyan))" }}/>
                  </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <Field label="Company Name *" span={2}><input name="companyName" value={form.companyName} onChange={handleChange} placeholder="Acme Corp" required className={iCls} style={iSty}/></Field>
            <Field label="Status"><Sel name="status" value={form.status} onChange={handleChange}>{STATUS_OPTIONS.map((s)=><option key={s.value} value={s.value}>{s.label}</option>)}</Sel></Field>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <Field label="Priority"><Sel name="priority" value={form.priority} onChange={handleChange}>{PRIORITY_OPTIONS.map((p)=><option key={p.value} value={p.value}>{p.label}</option>)}</Sel></Field>
            <Field label="Lead Source"><Sel name="leadSource" value={form.leadSource} onChange={handleChange}><option value="">Select source</option>{LEAD_SOURCES.map((s)=><option key={s} value={s}>{s}</option>)}</Sel></Field>
            <Field label="Follow-up Date"><input type="date" name="followUpDate" value={form.followUpDate??""} onChange={handleChange} className={iCls} style={iSty}/></Field>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <Field label="Country"><Sel name="country" value={form.country} onChange={handleChange}><option value="">Select country</option>{COUNTRIES.map((c)=><option key={c} value={c}>{c}</option>)}</Sel></Field>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Field label="Tags (comma separated)"><input value={tagInput} onChange={(e)=>setTagInput(e.target.value)} placeholder="hot, enterprise" className={iCls} style={iSty}/></Field>
          </div>
          <label className="flex items-center gap-3" style={{ cursor:"pointer" }}>
            <div className="relative w-10 h-6 rounded-full transition-colors duration-300 flex-shrink-0" style={{ background:form.hasWhatsapp?"linear-gradient(135deg, var(--accent), var(--cyan))":"var(--border2)" }} onClick={()=>field("hasWhatsapp",!form.hasWhatsapp)}>
              <span className="absolute top-1 w-4 h-4 rounded-full bg-white shadow transition-all duration-300" style={{ left:form.hasWhatsapp?"calc(100% - 20px)":"4px" }}/>
            </div>
            <span className="text-[13px] font-body" style={{ color:"var(--ink2)" }}>WhatsApp number is active</span>
          </label>
          <Field label="Notes / Proposal">
            <textarea name="proposal" value={form.proposal} onChange={handleChange} rows={4} placeholder="Notes, context, next steps…"
          ...
          {error && <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl border text-[13px]" style={{ background:"rgba(239,68,68,0.07)", borderColor:"rgba(239,68,68,0.2)", color:"#FF6B6B" }}>{error}</div>}
          <div className="flex gap-3 pt-2 sticky bottom-0 -mx-6 px-6 pb-6 mt-auto" style={{ background:"var(--bg-panel)", borderTop:"1px solid var(--border)" }}>
            <button type="button" onClick={onClose} className="flex-1 py-3 rounded-xl text-[14px] font-medium transition-all border" style={{ borderColor:"var(--border2)", color:"var(--ink3)", cursor:"pointer", background:"transparent" }}
              onMouseEnter={(e)=>{ (e.currentTarget as HTMLElement).style.background="var(--bg-alt)"; }}
              onMouseLeave={(e)=>{ (e.currentTarget as HTMLElement).style.background="transparent"; }}>Cancel</button>
            <button type="submit" disabled={saving||uploading} className="flex-1 py-3 rounded-xl text-[14px] font-semibold text-white disabled:opacity-60 disabled:cursor-not-allowed transition-all" style={{ background:"linear-gradient(135deg, var(--accent), var(--cyan))", cursor:"pointer" }}>
              {saving ? <span className="flex items-center justify-center gap-2"><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"/>Saving…</span> : isEdit ? "Save Changes" : "Add Lead →"}
            </button>
          </div>
        </form>
      </div>
...
function DeleteConfirm({ name, onConfirm, onCancel }: { name:string; onConfirm:()=>void; onCancel:()=>void }) {
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center px-4" style={{ background:"rgba(0,0,0,0.65)", backdropFilter:"blur(6px)" }}>
      <div className="w-full max-w-[380px] rounded-2xl p-7 text-center" style={{ background:"var(--bg-surface)", border:"1px solid var(--border2)", animation:"fadeScaleIn .25s cubic-bezier(0.16,1,0.3,1) both" }}>
        <div className="w-12 h-12 rounded-2xl flex items-center justify-center mx-auto mb-4" style={{ background:"rgba(239,68,68,0.1)", border:"1px solid rgba(239,68,68,0.2)" }}>
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M3 7h14M8 7V4.5h4V7M6 7l1 11h6l1-11" stroke="#EF4444" strokeWidth="1.5" strokeLinecap="round"/></svg>
        </div>
        <h3 className="font-display text-[17px] font-bold mb-2" style={{ color:"var(--ink)" }}>Delete Lead?</h3>
        <p className="text-[13px] mb-6 font-body" style={{ color:"var(--ink4)" }}>"<strong style={{ color:"var(--ink3)" }}>{name}</strong>" will be permanently removed.</p>
        <div className="flex gap-3">
          <button onClick={onCancel} className="flex-1 py-2.5 rounded-xl text-[13px] border" style={{ borderColor:"var(--border2)", color:"var(--ink3)", cursor:"pointer" }}>Cancel</button>
          <button onClick={onConfirm} className="flex-1 py-2.5 rounded-xl text-[13px] font-semibold text-white" style={{ background:"linear-gradient(135deg, #e53535, #EF4444)", cursor:"pointer" }}>Delete</button>
        </div>
      </div>
    </div>
  );
...
function LeadModal({ lead, onClose, onEdit, onAudit, onSendEmail, aiLoading }: {
  lead:FirestoreLead; onClose:()=>void; onEdit:()=>void;
  onAudit:(id:string)=>void; onSendEmail:(id:string, type:FollowUpKey, customSubject?:string, customBody?:string)=>void; aiLoading:boolean;
}) {
  const [tab, setTab]     = useState<"details"|"emails"|"notes">("details");

  // ── Email composer state ──────────────────────────────────────────────────
  const [composing,      setComposing]      = useState(false);   // editor open/close
  const [genLoading,     setGenLoading]     = useState(false);   // AI generating
  ...
  // Generate AI draft and open composer
  ...
  function handleConfirmSend() {
    ...
    onSendEmail(lead.id!, pendingType, editSubject, editBody);
    setComposing(false);
    setPendingType(null);
  }

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 py-8" style={{ background:"rgba(0,0,0,0.65)", backdropFilter:"blur(8px)" }} onClick={(e)=>e.target===e.currentTarget&&onClose()}>
      <div className="w-full max-w-[680px] max-h-[90vh] overflow-y-auto rounded-2xl" style={{ background:"var(--bg-surface)", border:"1px solid var(--border2)", boxShadow:"0 32px 80px rgba(0,0,0,0.45)", animation:"fadeScaleIn .3s cubic-bezier(0.16,1,0.3,1) both" }}>
        <div className="flex items-start justify-between p-6 pb-4" style={{ borderBottom:"1px solid var(--border)" }}>
          <div className="flex items-start gap-4 flex-1 min-w-0">
            {lead.imageUrl
              ? <img src={lead.imageUrl} alt={lead.companyName} className="w-12 h-12 rounded-xl object-contain flex-shrink-0" style={{ background:"var(--bg-panel)", border:"1px solid var(--border2)", padding:4 }}/>
              : <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 font-display text-xl font-bold text-white" style={{ background:"linear-gradient(135deg, var(--accent), var(--cyan))" }}>{lead.companyName[0]}</div>
            }
            <div className="min-w-0">
              <h2 className="font-display text-[20px] font-bold tracking-tight truncate" style={{ color:"var(--ink)" }}>{lead.companyName}</h2>
              <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                ...
                <span className="font-mono text-[10px]" style={{ color:"var(--ink4)" }}>Score: <strong style={{ color:score>70?"#10B981":score>40?"#F59E0B":"#EF4444" }}>{score}</strong></span>
              </div>
            </div>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0" style={{ color:"var(--ink4)", cursor:"pointer" }}
            onMouseEnter={(e)=>{ (e.currentTarget as HTMLElement).style.background="var(--bg-alt)"; }}
            onMouseLeave={(e)=>{ (e.currentTarget as HTMLElement).style.background="transparent"; }}>
            <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><path d="M2 2l9 9M11 2L2 11" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg>
          </button>
        ...
        <div className="flex gap-1 px-6 pt-4">
          {(["details","emails","notes"] as const).map((t) => (
            <button key={t} onClick={()=>setTab(t)} className="px-4 py-2 rounded-xl font-mono text-[11px] tracking-[0.08em] uppercase transition-all duration-200 capitalize"
        ...
        <div className="p-6">
            ...
            <div className="flex flex-col gap-4">
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  { label:"Country",  val:lead.country },
                  { label:"City",     val:lead.city },
                  { label:"Source",   val:lead.leadSource },
                  { label:"Assigned", val:lead.employeeName },
                  { label:"Follow-up", val:lead.followUpDate?new Date(lead.followUpDate).toLocaleDateString():null },
                  { label:"Social",   val:lead.socialLinks },
                ].filter((r)=>r.val).map(({ label, val, href })=>(
                  <div key={label} className="flex flex-col gap-1 p-3 rounded-xl" style={{ background:"var(--bg-alt)", border:"1px solid var(--border)" }}>
                    ...
                    {href ? <a href={href} target="_blank" rel="noopener noreferrer" className="text-[13px] font-body truncate" style={{ color:"var(--accent)" }}>{val}</a>
              ...
              <div className="p-4 rounded-xl" style={{ background:"var(--bg-alt)", border:"1px solid var(--border)" }}>
                <div className="flex items-center justify-between mb-3">
                  ...
                  {lead.website && (
                    <button onClick={()=>onAudit(lead.id!)} disabled={aiLoading}
                ...
                {audit ? (
                  <div className="flex flex-col gap-2">
                    ...
                    <span className="font-mono text-[10px]" style={{ color:"var(--ink4)" }}>Score: <strong style={{ color:audit.score>70?"#10B981":audit.score>40?"#F59E0B":"#EF4444" }}>{audit.score}/100</strong></span>
                  </div>
                ) : <p className="text-[12px] font-body" style={{ color:"var(--ink4)" }}>{lead.website?"Click 'Run Audit' to analyse":"No website provided"}</p>}
              </div>
              {lead.tags?.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {lead.tags.map((t)=><span key={t} className="font-mono text-[10px] px-2.5 py-1 rounded-xl" style={{ background:"var(--bg-panel)", border:"1px solid var(--border2)", color:"var(--ink4)" }}>{t}</span>)}
                </div>
              )}
              {lead.proposal && (
                <div className="p-4 rounded-xl" style={{ background:"var(--bg-alt)", border:"1px solid var(--border)" }}>
          ...
          {tab === "emails" && (
            <div className="flex flex-col gap-4">
              ...
              {/* ── COMPOSER (open after AI generates) ── */}
              {composing ? (
                <div className="flex flex-col gap-3 p-4 rounded-2xl" style={{ background:"var(--bg-alt)", border:"1px solid var(--accent-pale2)" }}>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={handleCancelCompose} className="font-mono text-[10px] px-2.5 py-1 rounded-lg" style={{ color:"var(--ink4)", border:"1px solid var(--border2)", cursor:"pointer", background:"transparent" }}>✕ Cancel</button>
                  </div>
                  ...
                  <div className="flex gap-2 pt-1">
                    <button
                      onClick={()=>pendingType && handleOpenCompose(pendingType)}
                      ...
                      <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M1 6A5 5 0 1 0 6 1" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round"/><path d="M1 1v5h5" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                      Regenerate
                    </button>
                    <button
                      ...
                      style={{ background:"linear-gradient(135deg, var(--accent), var(--cyan))", cursor:(genLoading||aiLoading)?"wait":"pointer", boxShadow:"0 4px 16px var(--accent-dim)" }}>
                      <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><rect x="1" y="2.5" width="11" height="8" rx="1.5" stroke="white" strokeWidth="1.1" fill="none"/><path d="M1 3.5l5.5 4 5.5-4" stroke="white" strokeWidth="1.1" fill="none"/></svg>
                ...
                /* ── SEND BUTTONS (before composer opens) ── */
                nextEmail ? (
                  <div className="flex flex-col gap-2">
                    {/* Quick send */}
                    <button
                      onClick={()=>onSendEmail(lead.id!, nextEmail)}
                      disabled={aiLoading}
                      ...
                      style={{ background:"linear-gradient(135deg, var(--accent), var(--cyan))", cursor:aiLoading?"wait":"pointer", boxShadow:"0 4px 16px var(--accent-dim)" }}>
                      <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><rect x="1" y="3" width="12" height="8" rx="1.5" stroke="white" strokeWidth="1.2" fill="none"/><path d="M1 4l6 3.5L13 4" stroke="white" strokeWidth="1.2" fill="none"/></svg>
                      {aiLoading ? "Generating…" : `⚡ Quick Send — ${FOLLOW_UP_SEQUENCE.find((s)=>s.key===nextEmail)?.label}`}
                    </button>
                    {/* Customize then send */}
                    <button
                      onClick={()=>handleOpenCompose(nextEmail)}
                      disabled={aiLoading}
                      ...
                      <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><path d="M9.5 1.5l2 2L5 10H3V8L9.5 1.5z" stroke="currentColor" strokeWidth="1.1" strokeLinejoin="round"/></svg>
                  ...
                  <div className="text-center py-3 rounded-xl font-mono text-[12px]" style={{ background:"rgba(34,197,94,0.08)", color:"#86efac", border:"1px solid rgba(34,197,94,0.2)" }}>
                  ...
                  <div className="relative pl-6">
                    ...
                    {mailHist.map((mail, i) => (
                      <div key={i} className="relative pb-5 last:pb-0">
                        <div className="absolute -left-[17px] top-1.5 w-2.5 h-2.5 rounded-full border-2" style={{ borderColor:"var(--bg-surface)", background:mail.status==="sent"?"#22c55e":"#ef4444" }}/>
                        <div className="flex items-center gap-3 mb-1">
                          <span className="font-display text-[13px] font-semibold" style={{ color:"var(--ink)" }}>{FOLLOW_UP_SEQUENCE.find((s)=>s.key===mail.type)?.label??mail.type}</span>
                          <span className="font-mono text-[10px]" style={{ color:"var(--ink4)" }}>{fmtDate(mail.date)}</span>
                          <span className="font-mono text-[9px] px-2 py-0.5 rounded-md uppercase font-bold" style={{ color:mail.status==="sent"?"#86efac":"#fca5a5", background:mail.status==="sent"?"rgba(34,197,94,0.08)":"rgba(239,68,68,0.08)" }}>{mail.status}</span>
                        </div>
                        ...
                        {mail.bodySnapshot && (
                          <details className="mt-1">
                            ...
                            <div className="mt-1.5 p-3 rounded-lg text-[11px] font-body leading-relaxed whitespace-pre-wrap max-h-40 overflow-y-auto" style={{ color:"var(--ink3)", background:"var(--bg-alt)", border:"1px solid var(--border)" }}>{mail.bodySnapshot}</div>
                          </details>
                        )}
                      </div>
                    ))}
                  ...
                  {lead.notes.map((n, i) => (
                    <div key={i} className="p-4 rounded-xl" style={{ background:"var(--bg-alt)", border:"1px solid var(--border)" }}>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="font-mono text-[10px]" style={{ color:"var(--ink4)" }}>{n.date?new Date(n.date).toLocaleDateString():""}</span>
                      </div>
        ...
        <div className="flex gap-3 px-6 pb-6 pt-5" style={{ borderTop:"1px solid var(--border)" }}>
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl text-[13px] font-medium border" style={{ borderColor:"var(--border2)", color:"var(--ink3)", cursor:"pointer" }}>Close</button>
          <button onClick={onEdit} className="flex-1 py-2.5 rounded-xl text-[13px] font-semibold text-white" style={{ background:"linear-gradient(135deg, var(--accent), var(--cyan))", cursor:"pointer" }}>Edit Lead →</button>
        </div>
      </div>
    </div>
  );
...
// ══════════════════════════════════════════════════════════════════════════════
// AI SEARCH MODAL — Production Google Maps Scraper
// ══════════════════════════════════════════════════════════════════════════════
function AiSearchModal({ onClose, onImport }: { onClose:()=>void; onImport:(leads:Partial<FirestoreLead>[])=>Promise<void> }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4" style={{ background:"rgba(0,0,0,0.7)", backdropFilter:"blur(8px)" }} onClick={onClose}>
      <div className="w-full max-w-[620px] max-h-[85vh] overflow-y-auto rounded-2xl p-6" style={{ background:"var(--bg-surface)", border:"1px solid var(--border2)", animation:"fadeScaleIn .25s cubic-bezier(0.16,1,0.3,1) both" }} onClick={(e)=>e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between mb-5">
          ...
          <button onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ color:"var(--ink4)", cursor:"pointer" }}>✕</button>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          <div className="flex flex-col gap-1">
            ...
            <input value={keyword} onChange={(e)=>setKeyword(e.target.value)} placeholder="e.g. Real Estate Agency"
              ...
              style={{ background:"var(--bg-alt)", color:"var(--ink)" }} onKeyDown={(e)=>e.key==="Enter"&&handleSearch()}/>
          </div>
          <div className="flex flex-col gap-1">
        ...
        <button onClick={handleSearch} disabled={searching||!keyword||!location}
          ...
          style={{ background:"linear-gradient(135deg, var(--accent), var(--cyan))", cursor:searching?"wait":"pointer" }}>
          {searching
        ...
        {!HASDATA_KEY && !OUTSCRAPER_KEY && (
          <div className="mb-4 px-3 py-2.5 rounded-xl border text-[11px] font-mono" style={{ background:"rgba(245,158,11,0.07)", borderColor:"rgba(245,158,11,0.2)", color:"#fde68a" }}>
          ...
          <>
            <div className="flex items-center justify-between px-4 py-2.5 rounded-t-xl"
              ...
              <div className="flex items-center gap-3">
                <span className="font-mono text-[10px] uppercase tracking-widest" style={{ color:"var(--ink4)" }}>{results.length} businesses found</span>
                {srcInfo && (
              ...
              <button onClick={()=>setResults((p)=>p.map((r)=>({ ...r, selected:!p.every((x)=>x.selected) })))}
                className="font-mono text-[10px]" style={{ color:"var(--accent)", cursor:"pointer", background:"none", border:"none" }}>
                Toggle All
              </button>
            </div>
            ...
            <button onClick={handleImport} disabled={importing||results.filter((r)=>r.selected).length===0}
              ...
              style={{ background:"linear-gradient(135deg, var(--accent), var(--cyan))", cursor:importing?"wait":"pointer" }}>
...
function ComposeModal({ leads, onClose, onSend }: { leads:FirestoreLead[]; onClose:()=>void; onSend:(leads:FirestoreLead[], type:FollowUpKey)=>Promise<void> }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled) { setPreview("(Preview failed — leads will still get personalised emails)"); setSubject(`Follow-up for ${leads[0].companyName}`); }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4" style={{ background:"rgba(0,0,0,0.65)", backdropFilter:"blur(8px)" }} onClick={onClose}>
      <div className="w-full max-w-[580px] max-h-[85vh] overflow-y-auto rounded-2xl p-6" style={{ background:"var(--bg-surface)", border:"1px solid var(--border2)", animation:"fadeScaleIn .25s cubic-bezier(0.16,1,0.3,1) both" }} onClick={(e)=>e.stopPropagation()}>
        <div className="flex items-center justify-between mb-5">
          <div>
            <h2 className="font-display text-[17px] font-bold" style={{ color:"var(--ink)" }}>📧 AI Email Composer</h2>
            <p className="font-mono text-[11px] mt-1" style={{ color:"var(--ink4)" }}>Sending to <strong style={{ color:"var(--ink)" }}>{leads.length}</strong> lead{leads.length!==1?"s":""}</p>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ color:"var(--ink4)", cursor:"pointer" }}>✕</button>
        </div>
        <div className="flex flex-wrap gap-2 mb-5">
          {FOLLOW_UP_SEQUENCE.map((step) => (
            <button key={step.key} onClick={()=>setEmailType(step.key)} className="px-3 py-2 rounded-xl font-mono text-[11px] font-semibold transition-all duration-150"
        ...
        <div className="mb-5">
          <div className="flex items-center justify-between mb-2">
            <span className="font-mono text-[10px] uppercase tracking-widest" style={{ color:"var(--ink4)" }}>Preview ({leads[0]?.companyName})</span>
            {loading && <span className="flex items-center gap-1.5"><div className="w-3 h-3 border border-current border-t-transparent rounded-full animate-spin" style={{ color:"var(--accent)" }}/><span className="font-mono text-[10px]" style={{ color:"var(--accent)" }}>Generating...</span></span>}
          </div>
          <div className="px-3 py-2 rounded-t-xl font-mono text-[12px]" style={{ background:"var(--bg-alt)", border:"1px solid var(--border2)", borderBottom:"none", color:"var(--ink3)" }}>
            <span style={{ color:"var(--ink4)" }}>Subject:</span> <span style={{ color:"var(--ink)" }}>{loading?"...":subject}</span>
          </div>
          <div className="p-4 rounded-b-xl font-body text-[12px] leading-relaxed whitespace-pre-wrap max-h-48 overflow-y-auto" style={{ background:"var(--bg-alt)", border:"1px solid var(--border2)", color:loading?"var(--ink4)":"var(--ink3)" }}>
        ...
        <div className="flex justify-end gap-3">
          <button onClick={onClose} className="px-5 py-2.5 rounded-xl text-[13px] font-medium" style={{ border:"1px solid var(--border2)", color:"var(--ink3)", cursor:"pointer", background:"transparent" }}>Cancel</button>
          <button onClick={handleSend} disabled={sending||loading} className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-[13px] font-semibold text-white transition-all disabled:opacity-50" style={{ background:"linear-gradient(135deg, var(--accent), var(--cyan))", cursor:sending?"wait":"pointer" }}>
            {sending ? <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"/>Sending...</> : `Send ${leads.length} Email${leads.length!==1?"s":""}`}
          </button>
        </div>
      </div>
    </div>
...
export function LeadTab({ showToast }: Props) {
  ...
  const toggleSelectAll = (ids:string[]) => setSelectedIds((p) => {
    const all = ids.every((id)=>p.has(id)); const n=new Set(p);
    all ? ids.forEach((id)=>n.delete(id)) : ids.forEach((id)=>n.add(id)); return n;
  });
  ...
  async function handleAudit(leadId:string) {
    ...
    try {
      ...
      await updateLead(leadId, { auditData:result } as any);
      await load();
      showToast(`Audit done — Score: ${result.score}/100`);
    } catch (err:any) {
      showToast(err?.message?.includes("key")?"API key missing — check .env":"AI audit failed","error");
    } finally { setAiLoading(null); }
  }

  // ── AI: Generate + Send Email ──────────────────────────────────────────────
  async function handleSendEmail(leadId:string, emailType:FollowUpKey, customSubject?:string, customBody?:string) {
    const lead = leads.find((l)=>l.id===leadId);
    if (!lead) return;
    ...
    try {
      ...
      // Agar custom content nahi hai toh AI se generate karo
      if (!subject || !body) {
      ...
      const history = [...getMailHistory(lead)];
      history.push({ date:today(), type:emailType, status, subject, bodySnapshot:body });
      ...
      else showToast(`Email generated but send failed: ${sendResult.error}`,"error");

    } catch (err:any) {
      showToast(err?.message?.includes("key")?"API key missing":"Email generation failed","error");
    } finally { setAiLoading(null); }
  }
    ...
    try {
      ...
      for (let i = 0; i < targetLeads.length; i++) {
        ...
        try {
          ...
          const status: "sent"|"failed" = sendResult.success ? "sent" : "failed";
          const history = [...getMailHistory(lead)];
          history.push({ date:today(), type:emailType, status, subject:content.subject, bodySnapshot:content.body });
          await updateLead(lead.id!, { mailHistory:history, lastContacted:today(), followUpCount:(lead.followUpCount??0)+1, status:lead.status==="new"?"contacted":lead.status } as any);
  ...
  async function handleAiImport(newLeads:Partial<FirestoreLead>[]) {
    for (const lead of newLeads) await createLead(lead as any);
    await load();
  ...
  async function handleDelete() {
    ...
    try { await deleteLead(deleteTgt.id); setDeleteTgt(null); await load(); showToast(`"${deleteTgt.companyName}" deleted`); }
  ...
  const filtered = leads.filter((l) => {
    ...
    const ms = !q||l.companyName?.toLowerCase().includes(q)||l.email?.toLowerCase().includes(q)||l.phone?.includes(q)||l.country?.toLowerCase().includes(q)||l.city?.toLowerCase().includes(q)||l.tags?.some((t)=>t.toLowerCase().includes(q));
    return ms && (statusF==="all"||l.status===statusF) && (priorityF==="all"||l.priority===priorityF) && (countryF==="all"||l.country===countryF);
  });

  const dueFollowUps = useMemo(()=>leads.filter((l)=>getDueFollowUp(l)!==null&&l.status!=="closed"&&l.status!=="lost"),[leads]);
  const stats = {
    total:leads.length, new:leads.filter((l)=>l.status==="new").length,
    closed:leads.filter((l)=>l.status==="closed").length, high:leads.filter((l)=>l.priority==="high").length,
    emailsSent:leads.reduce((a,l)=>a+getMailHistory(l).length,0), dueFollowUps:dueFollowUps.length,
  };
  const countries = [...new Set(leads.map((l)=>l.country).filter(Boolean))].sort() as string[];
  ...
  return (
    <div className="p-6 flex flex-col gap-5">
      ...
      {!AI_ENABLED && (
        <div className="flex items-center gap-3 px-4 py-3 rounded-xl border" style={{ background:"rgba(245,158,11,0.08)", borderColor:"rgba(245,158,11,0.2)", color:"#fde68a" }}>
          <span>⚠️</span>
          <span className="font-mono text-[11px]">AI features disabled — add <strong>VITE_OPENROUTER_API_KEY</strong> to .env and restart. Get free key: <a href="https://openrouter.ai/keys" target="_blank" rel="noopener" style={{ color:"var(--accent)" }}>openrouter.ai/keys</a></span>
        </div>
      )}
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex rounded-xl p-1" style={{ background:"var(--bg-alt)", border:"1px solid var(--border2)" }}>
          {([
            ...
            <button key={id} onClick={()=>setSubTab(id)} className="flex items-center gap-2 px-4 py-2 rounded-lg font-mono text-[11px] font-semibold transition-all duration-200"
              style={{ cursor:"pointer", background:subTab===id?"linear-gradient(135deg, var(--accent), var(--cyan))":"transparent", color:subTab===id?"white":"var(--ink4)" }}>
              {label}
              {count!==null&&count>0&&<span className="px-1.5 py-0.5 rounded-md text-[9px] font-bold" style={{ background:subTab===id?"rgba(255,255,255,0.2)":"var(--bg-surface)", color:subTab===id?"white":"var(--ink4)" }}>{count}</span>}
            </button>
          ))}
        </div>
        <div className="flex items-center gap-2">
          {selectedLeads.length > 0 && <>
            <button onClick={()=>setShowCompose(true)} className="flex items-center gap-2 px-4 py-2 rounded-xl text-[12px] font-semibold text-white" style={{ background:"linear-gradient(135deg, var(--accent), var(--cyan))", cursor:"pointer" }}>📧 Email {selectedLeads.length}</button>
            <button onClick={()=>setSelectedIds(new Set())} className="font-mono text-[10px] px-3 py-2 rounded-xl" style={{ color:"var(--ink4)", cursor:"pointer", background:"none", border:"1px solid var(--border2)" }}>Clear</button>
          </>}
          {AI_ENABLED && <button onClick={()=>setShowAiSearch(true)} className="flex items-center gap-2 px-4 py-2 rounded-xl text-[12px] font-semibold transition-all" style={{ border:"1px solid var(--border2)", color:"var(--ink3)", cursor:"pointer", background:"transparent" }}>🤖 AI Search</button>}
          <button onClick={()=>{ setEditLead(null); setFormOpen(true); }} className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-[13px] font-semibold text-white transition-all active:scale-95" style={{ background:"linear-gradient(135deg, var(--accent), var(--cyan))", boxShadow:"0 4px 16px var(--accent-dim)", cursor:"pointer" }}>
            <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M6 1v10M1 6h10" stroke="white" strokeWidth="1.8" strokeLinecap="round"/></svg>Add Lead
          </button>
        </div>
      </div>
        ...
        <>
          <div className="grid grid-cols-2 lg:grid-cols-6 gap-3">
            {([
              ...
              <div key={label} className="rounded-2xl p-4 flex flex-col gap-1.5 relative overflow-hidden" style={{ background:"var(--bg-surface)", border:"1px solid var(--border2)" }}>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="relative flex-1 min-w-[200px]">
              <div className="absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" style={{ color:"var(--ink4)" }}>
                <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><circle cx="5.5" cy="5.5" r="4" stroke="currentColor" strokeWidth="1"/><path d="M8.5 8.5l3 3" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round"/></svg>
              </div>
              <input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Search leads…" className="w-full pl-8 pr-4 py-2 rounded-xl text-[13px] border border-[var(--border2)] outline-none transition-colors" style={{ background:"var(--bg-surface)", color:"var(--ink)", cursor:"text" }}
                onFocus={(e)=>{ (e.target as HTMLInputElement).style.borderColor="var(--accent)"; }}
                onBlur={(e) =>{ (e.target as HTMLInputElement).style.borderColor="var(--border2)"; }}/>
            </div>
            <select value={statusF}   onChange={(e)=>setStatusF(e.target.value)}   className={selStyle} style={selInline}><option value="all">All Statuses</option>{STATUS_OPTIONS.map((s)=><option key={s.value} value={s.value}>{s.label}</option>)}</select>
            <select value={priorityF} onChange={(e)=>setPriorityF(e.target.value)} className={selStyle} style={selInline}><option value="all">All Priorities</option>{PRIORITY_OPTIONS.map((p)=><option key={p.value} value={p.value}>{p.label}</option>)}</select>
            {countries.length > 0 && <select value={countryF} onChange={(e)=>setCountryF(e.target.value)} className={selStyle} style={selInline}><option value="all">All Countries</option>{countries.map((c)=><option key={c} value={c}>{c}</option>)}</select>}
          ...
          <div className="rounded-2xl overflow-hidden" style={{ background:"var(--bg-surface)", border:"1px solid var(--border2)" }}>
            <div className="grid px-5 py-3" style={{ gridTemplateColumns:"36px 2fr 1fr 1fr 100px 100px 80px", background:"var(--bg-alt)", borderBottom:"1px solid var(--border2)" }}>
              <div><input type="checkbox" onChange={()=>toggleSelectAll(filtered.map((l)=>l.id!))} checked={filtered.length>0&&filtered.every((l)=>selectedIds.has(l.id!))} className="w-4 h-4" style={{ accentColor:"var(--accent)" }}/></div>
              {["Company","Status","Priority","Email Stage","Audit","Actions"].map((h)=><span key={h} className="font-mono text-[9px] tracking-[0.16em] uppercase" style={{ color:"var(--ink4)" }}>{h}</span>)}
            </div>
            {loading ? (
              <div className="flex flex-col items-center justify-center py-20 gap-4">
                <div className="w-9 h-9 rounded-full border-2 border-t-transparent animate-spin" style={{ borderColor:"var(--border2)", borderTopColor:"var(--accent)" }}/>
                <span className="font-mono text-[11px]" style={{ color:"var(--ink4)" }}>Loading leads…</span>
              </div>
            ) : filtered.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20"><p className="font-display text-[14px] font-semibold" style={{ color:"var(--ink3)" }}>{search||statusF!=="all"?"No results found":"No leads yet"}</p></div>
            ) : (
              filtered.map((lead) => {
                const st=getStatus(lead.status), pc=getPriorityColor(lead.priority);
                const score=lead.leadScore??calcScore(lead), audit=getAuditData(lead);
                const due=getDueFollowUp(lead), isSel=selectedIds.has(lead.id!);
                return (
                  <div key={lead.id} className="grid px-5 py-3.5 border-b last:border-0 group transition-colors duration-150"
                    ...
                    <div className="flex items-center"><input type="checkbox" checked={isSel} onChange={()=>toggleSelect(lead.id!)} className="w-4 h-4" style={{ accentColor:"var(--accent)" }}/></div>
                    <div className="flex items-center gap-3 min-w-0 cursor-pointer" onClick={()=>setViewLead(lead)}>
                      {lead.imageUrl
                        ? <img src={lead.imageUrl} alt={lead.companyName} className="w-9 h-9 rounded-xl object-contain flex-shrink-0" style={{ background:"var(--bg-panel)", border:"1px solid var(--border2)", padding:2 }}/>
                        : <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 font-bold text-white text-sm" style={{ background:"linear-gradient(135deg, var(--accent), var(--cyan))" }}>{lead.companyName?.[0]??"?"}</div>}
                      <div className="min-w-0">
                      ...
                      <span className="flex-shrink-0 w-6 h-6 rounded-full text-[9px] font-bold flex items-center justify-center ml-1" style={{ background:score>70?"rgba(16,185,129,0.12)":score>40?"rgba(245,158,11,0.12)":"rgba(239,68,68,0.12)", color:score>70?"#10B981":score>40?"#F59E0B":"#EF4444", border:`1px solid ${score>70?"rgba(16,185,129,0.25)":score>40?"rgba(245,158,11,0.25)":"rgba(239,68,68,0.25)"}` }}>{score}</span>
                    </div>
                    <div className="flex items-center"><span className="px-2 py-0.5 rounded-lg font-mono text-[9px] font-bold tracking-widest" style={{ background:st.bg, color:st.color }}>{st.label}</span></div>
                    <div className="flex items-center"><span className="flex items-center gap-1.5 font-mono text-[11px] font-semibold" style={{ color:pc }}><span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background:pc }}/>{lead.priority}</span></div>
                    <div className="flex items-center"><div className="flex flex-col gap-1"><MailStatusBadge lead={lead}/>{due&&<span className="font-mono text-[9px] font-bold" style={{ color:"#F59E0B" }}>⏰ {due.label} due</span>}</div></div>
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                      ...
                      ].map(({ title, onClick, danger, icon }) => (
                        <button key={title} onClick={onClick} title={title} className="w-7 h-7 rounded-lg flex items-center justify-center transition-all" style={{ color:"var(--ink4)", cursor:"pointer" }}
        ...
        <>
          ...
          <div className="flex items-center justify-between">
            ...
            {dueFollowUps.length > 0 && (
              <button onClick={async()=>{ for(const l of dueFollowUps){ const d=getDueFollowUp(l); if(d)await handleSendEmail(l.id!,d.key); } }} disabled={!!aiLoading}
                className="flex items-center gap-2 px-4 py-2 rounded-xl text-[12px] font-semibold text-white disabled:opacity-50" style={{ background:"linear-gradient(135deg, var(--accent), var(--cyan))", cursor:aiLoading?"wait":"pointer" }}>
                Send All Follow-ups
              </button>
            )}
          </div>
          {dueFollowUps.length === 0 ? (
            <div className="text-center py-16"><p className="font-display text-[14px] font-semibold" style={{ color:"var(--ink3)" }}>All caught up!</p><p className="font-mono text-[11px] mt-1" style={{ color:"var(--ink4)" }}>No follow-ups due right now</p></div>
          ) : (
            <div className="rounded-2xl overflow-hidden" style={{ background:"var(--bg-surface)", border:"1px solid var(--border2)" }}>
              {dueFollowUps.map((lead) => {
                const due=getDueFollowUp(lead)!, last=getMailHistory(lead).at(-1)!, days=daysAgo(last.date);
                ...
                return (
                  <div key={lead.id} className="flex items-center gap-4 px-5 py-3.5 border-b last:border-0" style={{ borderColor:"var(--border)" }}>
                    ...
                    <button onClick={()=>handleSendEmail(lead.id!,due.key)} disabled={!!aiLoading} className="px-3 py-1.5 rounded-lg text-[11px] font-semibold text-white disabled:opacity-50" style={{ background:"linear-gradient(135deg, var(--accent), var(--cyan))", cursor:aiLoading?"wait":"pointer" }}>Send</button>
                  </div>
                );
              })}
            </div>
                ...
                {leads.filter((l)=>l.status==="new"&&getMailHistory(l).length===0).map((lead) => (
                  <div key={lead.id} className="flex items-center gap-4 px-5 py-3.5 border-b last:border-0" style={{ borderColor:"var(--border)" }}>
                    ...
                    <div className="flex gap-2">
                      <button onClick={()=>setViewLead(lead)} className="px-3 py-1.5 rounded-lg text-[11px] font-semibold" style={{ border:"1px solid var(--border2)", color:"var(--ink3)", cursor:"pointer", background:"transparent" }}>👁 Review</button>
                      {lead.website && <button onClick={()=>handleAudit(lead.id!)} disabled={!!aiLoading} className="px-3 py-1.5 rounded-lg text-[11px] font-semibold disabled:opacity-50" style={{ border:"1px solid var(--border2)", color:"var(--ink3)", cursor:"pointer", background:"transparent" }}>🤖 Audit</button>}
                      <button onClick={()=>handleSendEmail(lead.id!,"initial")} disabled={!!aiLoading} className="px-3 py-1.5 rounded-lg text-[11px] font-semibold text-white disabled:opacity-50" style={{ background:"linear-gradient(135deg, var(--accent), var(--cyan))", cursor:aiLoading?"wait":"pointer" }}>📧 Send Initial</button>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {([
              ...
              <div key={label} className="rounded-2xl p-5 relative overflow-hidden" style={{ background:"var(--bg-surface)", border:"1px solid var(--border2)" }}>
                ...
                <div className="font-display text-[28px] font-bold leading-none relative z-10" style={{ background:`linear-gradient(135deg, ${color}, ${color}aa)`, WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", backgroundClip:"text" }}>{val}</div>
              </div>
            ))}
          </div>
          <div className="rounded-2xl p-6" style={{ background:"var(--bg-surface)", border:"1px solid var(--border2)" }}>
            ...
            <div className="flex gap-1 h-10 rounded-xl overflow-hidden mb-4">
              {[{ key:"none", label:"No Emails" }, ...FOLLOW_UP_SEQUENCE].map(({ key, label }) => {
                ...
                return <div key={key} className="flex items-center justify-center rounded-lg" style={{ width:`${pct}%`, background:sc.bg, border:`1px solid ${sc.border}`, color:sc.text, fontSize:11, fontWeight:700, fontFamily:"'JetBrains Mono', monospace" }} title={`${label}: ${count}`}>{count}</div>;
              })}
            </div>
            <div className="flex flex-wrap gap-x-5 gap-y-2">
              {[{ key:"none", label:"No Emails" }, ...FOLLOW_UP_SEQUENCE].map(({ key, label }) => {
                ...
                if(!count) return null;
                const sc=MAIL_STATUS_COLORS[key]??MAIL_STATUS_COLORS.none;
                return <div key={key} className="flex items-center gap-2"><div className="w-2.5 h-2.5 rounded" style={{ background:sc.text }}/><span className="font-mono text-[10px]" style={{ color:"var(--ink4)" }}>{label}: <strong style={{ color:sc.text }}>{count}</strong></span></div>;
              })}
            </div>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <div className="rounded-2xl p-6" style={{ background:"var(--bg-surface)", border:"1px solid var(--border2)" }}>
              ...
              {Object.entries(leads.reduce<Record<string,number>>((a,l)=>{ const s=l.leadSource||"Unknown"; a[s]=(a[s]||0)+1; return a; },{} )).sort(([,a],[,b])=>b-a).map(([src,cnt]) => {
                const max=Math.max(...Object.values(leads.reduce<Record<string,number>>((a,l)=>{ const s=l.leadSource||"Unknown"; a[s]=(a[s]||0)+1; return a; },{})),1);
                return <div key={src} className="mb-3"><div className="flex items-center justify-between mb-1.5"><span className="font-mono text-[11px]" style={{ color:"var(--ink3)" }}>{src}</span><span className="font-mono text-[11px] font-bold" style={{ color:"var(--accent)" }}>{cnt}</span></div><div className="h-2 rounded-full overflow-hidden" style={{ background:"var(--bg-alt)" }}><div className="h-full rounded-full transition-all duration-700" style={{ width:`${Math.round((cnt/max)*100)}%`, background:"linear-gradient(90deg, var(--accent), var(--cyan))" }}/></div></div>;
              })}
            </div>
            <div className="rounded-2xl p-6" style={{ background:"var(--bg-surface)", border:"1px solid var(--border2)" }}>
              ...
              {STATUS_OPTIONS.map(({ value, label, color }) => {
                ...
                return <div key={value} className="mb-3"><div className="flex items-center justify-between mb-1.5"><span className="font-mono text-[11px]" style={{ color:"var(--ink3)" }}>{label}</span><span className="font-mono text-[11px] font-bold" style={{ color }}>{cnt} ({pct}%)</span></div><div className="h-2 rounded-full overflow-hidden" style={{ background:"var(--bg-alt)" }}><div className="h-full rounded-full transition-all duration-700" style={{ width:`${pct}%`, background:`linear-gradient(90deg, ${color}, ${color}88)` }}/></div></div>;
              })}
            </div>
          </div>
        </>
